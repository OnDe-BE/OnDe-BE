...
public class CommentController {
    ...
    public CommentResponseDto createComment(@PathVariable Long postIdx, @RequestBody CommentRequestsDto requestDto, @AuthenticationPrincipal User user) {
        return commentService.createComment(postIdx, requestDto, user);
    }

    // 댓글 수정
    @PutMapping("/modify/{commentIdx}")    // 여기서 ID는 댓글의 id
    public CommentResponseDto updateComment(@PathVariable Long commentIdx, @RequestBody CommentRequestsDto requestDto, @AuthenticationPrincipal User user) {
        return commentService.updateComment(commentIdx, requestDto, user);
    }

    // 댓글 삭제
    @DeleteMapping("/delete/{commentIdx}")     // 여기서 ID는 댓글의 id
    public SuccessResponseDto deleteComment(@PathVariable Long commentIdx, @AuthenticationPrincipal User user) {
